...
import { formatJsonRpcError, formatJsonRpcResult, JsonRpcRequest } from "@json-rpc-tools/utils";
...
export class EIP155SignerConnection implements IJsonRpcConnection {
      ...
      switch (request.method) {
        ...
        case "eth_sendTransaction":
          if (request.params[0]?.from.toLowerCase() !== address.toLowerCase()) {
            throw new Error(`Method ${request.method} targetted incorrect account: ${address}`);
          }
  ...
  private async register(url = this.url): Promise<Wallet> {
    ...
    const wallet = new Wallet(this.keyPair.privateKey, new providers.JsonRpcProvider(this.url));
    this.onOpen(wallet);
    return wallet;
  }
